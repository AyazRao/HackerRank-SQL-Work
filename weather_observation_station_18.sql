/*
Problem Statement:
Consider P1(a,b) and P2(c,d) to be two points on a 2D plane.
• a = minimum value in Northern Latitude (LAT_N in STATION).
• b = minimum value in Western Longitude (LONG_W in STATION).
• c = maximum value in Northern Latitude (LAT_N in STATION).
• d = maximum value in Western Longitude (LONG_W in STATION).
Query the Manhattan Distance between points P1 and P2 and round it to a scale of 4 decimal places.

Table Schema (STATION):
+----------+--------------+
| Field    | Type         |
+----------+--------------+
| ID       | NUMBER       |
| CITY     | VARCHAR2(21) |
| STATE    | VARCHAR2(2)  |
| LAT_N    | NUMBER       |
| LONG_W   | NUMBER       |
+----------+--------------+

Language: MySQL
*/

-- MySQL Query: Calculate the Manhattan Distance
SELECT
    -- Calculate Manhattan distance: |a - c| + |b - d| and round to 4 decimal places
    ROUND(
        ABS(MAX(LAT_N) - MIN(LAT_N)) -- |c - a|: Latitude difference
        +
        ABS(MAX(LONG_W) - MIN(LONG_W)), -- |d - b|: Longitude difference
    4)
FROM
    STATION;